...
%     dimension)
        ...
        rho = phi * phi';
%     Choi Channel from  Aux AO to Bi
        if dBI < dAO*d_aux
            d_aux_o = (dAO*d_aux)/dBI;
            D = PartialTrace(RandomSuperoperator([d_aux*dAO dBI*d_aux_o],1,0,0,1),4, [d_aux dAO dBI d_aux_o]);
        else
            D = RandomSuperoperator([d_aux*dAO dBI], 1,0,0,1); 
        end

%     Process [Ai Aux, Ao Bi AuxO] * [Ai, Aux AO BI AuxO]
        term_1 = Tensor(PartialTranspose(rho,2,[dAI d_aux]),eye(dAO*dBI));
        term_2 = Tensor(eye(dAI), D);
        W = PartialTrace(term_1 * term_2, 2, [dAI d_aux dAO dBI]);
